-- 0009: Reject an out-of-range promotion year in promote_grades_annual.
-- The function trusted p_promotion_year from the caller. A far-future year would promote once
-- then latch forward, freezing all later promotions; absurd years are nonsense. Bound it to
-- [2020 .. current WIB year]. The existing v_last_year >= p check still no-ops duplicates, and
-- catch-up after a skipped year still passes (p == current year > last). Body is otherwise
-- identical to 0002. CREATE OR REPLACE keeps the signature, so grants/types are unaffected.
...
DECLARE
  ...
  v_current_year INTEGER := EXTRACT(YEAR FROM (NOW() AT TIME ZONE 'Asia/Jakarta'))::INTEGER;
BEGIN
  -- Validate the caller-supplied year before reading or mutating anything.
  IF p_promotion_year < 2020 OR p_promotion_year > v_current_year THEN
    RAISE EXCEPTION 'invalid promotion year %: must be between 2020 and the current WIB year %',
      p_promotion_year, v_current_year USING ERRCODE = 'check_violation';
  END IF;
