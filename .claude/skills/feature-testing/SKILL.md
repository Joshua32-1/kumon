...
**Reminders** — `{"slot": 1}` runs Phase 1 (latest-due send) up to `WHATSAPP_BATCH_LIMIT`; `{"slot": 10}` adds the Phase 2 overdue chase. Phase 1 selects every invoice with a due (`scheduled_date <= today`) unsent reminder and, per invoice, sends only the **highest**-numbered due reminder while cancelling the lower due rows (`message_preview = "Digantikan pengingat terbaru"`). Verify with a seeded invoice whose reminders are `1:PENDING@<past>`, `2:PENDING@<past>`, `3:PENDING@<future>` and `date` between #2 and #3 → #2 `SENT`, #1 `CANCELLED`, #3 untouched. A `FAILED` row is retried for `REMINDER_FAILED_RETRY_WINDOW_DAYS` (11) — set `2:FAILED@<today>` → it sends; set `2:FAILED@<today − 11d>` → it still sends; set `2:FAILED@<today − 12d>` → it does **not** (the bound that stops a permanently-failing number being re-hit forever). A reminder whose invoice is already paid/cancelled/waived is not picked up at all (the discovery query inner-joins `invoices` on `PENDING`/`OVERDUE`), so it costs no batch slot — verify by marking an invoice paid with a `FAILED` reminder still present and re-running the slot: `processed` must not count it, and the invoice keeps showing its `send_failed` WhatsApp badge. Re-run the same slot: sent count must be 0 (no due-unsent rows remain). **Cross-slot dedupe:** on a reminder day, seed an OVERDUE prior-month invoice with a `FAILED` row dated the previous reminder day, run `{"slot": 1}` (Phase 1 sends it), then run `{"slot": 3}` — Phase 2 must **not** message that parent again (it skips invoices whose last send is already today). Stranded check: an unpaid invoice left with a `PENDING` reminder dated in the past surfaces as a delivery-attention ("Perlu tindakan (WA)") item. Messages must contain **no** "pertama/kedua/ketiga" ordinal. (Sends go through the real Meta API — seed test contacts with a number you control.)
