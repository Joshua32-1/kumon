...
`0015` reminder first-failure anchor — adds `payment_reminders.first_failed_on DATE` (nullable) plus a guarded backfill, so the Phase 1 FAILED retry window measures from the day a send was actually attempted rather than from `scheduled_date`. Those differ whenever the schedule and the cron cadence drift (a `reminder_days` config `vercel.json` does not follow, or a cron day that never ran), and a schedule-anchored window could expire before the next run. Verify: `SELECT column_name FROM information_schema.columns WHERE table_name = 'payment_reminders' AND column_name = 'first_failed_on';` returns 1 row.
...
### payment_reminders
Scheduled WhatsApp reminders, created at invoice generation for the 1st/11th/21st. FK `invoice_id` and `student_id` (both CASCADE). Columns: `reminder_number` (1–3, internal slot/order index — not shown to parents), `scheduled_date`, `sent_at`, `first_failed_on` (0015 — WIB day of the **first** failed send, stamped once and never refreshed, so the retry window cannot slide forward forever), `status` (`reminder_status`), `whatsapp_number` (snapshot), `message_preview`. The send cron processes each invoice with a due (`scheduled_date <= today`) unsent reminder and sends only its **latest** due reminder, marking earlier due rows `CANCELLED` ("Digantikan pengingat terbaru") so a reminder stranded in the past (mid-month enrollment / cuti rebill) can't fire later or out of order; `SENT` rows are skipped by later slots. A `FAILED` row stays retryable for `REMINDER_FAILED_RETRY_WINDOW_DAYS` (11) past the later of `first_failed_on` and `scheduled_date` (so rows predating 0015, and rows failed early by the admin push-ahead, keep the `scheduled_date` anchor) — one further cron run — then ages out. Discovery joins `invoices` and considers only `PENDING`/`OVERDUE` ones, so rows left behind on a settled invoice (settlement cancels `PENDING` rows but deliberately leaves `FAILED` ones, which are the only record of why a send was rejected) never consume send capacity.
