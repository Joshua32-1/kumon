...
Slot semantics: every slot runs Phase 1 — for each invoice with a due (`scheduled_date <= today`) unsent reminder, send only its latest due reminder and cancel earlier due rows ("Digantikan pengingat terbaru"); slots ≥ `REMINDER_PHASE2_START_SLOT` (3) additionally chase overdue/prior months (Phase 2). `PENDING` rows are due with no lower bound; `FAILED` rows are retried only while the later of `first_failed_on` (the day the send was actually attempted) and `scheduled_date` is within `REMINDER_FAILED_RETRY_WINDOW_DAYS` (11) of `today` — far enough to reach the next reminder day, so a failure mid-outage is retried rather than dropped, and no further, so a permanently-failing number is eventually left alone. Phase 1 always drains first inside an invocation, so Phase 2 only spends leftover budget. Constants in [lib/constants.ts](lib/constants.ts).

Phase 2 reaches back at most `REMINDER_CHASE_MAX_PRIOR_MONTHS` (3) billing periods — the query filters to that window server-side, so the scanned set stays flat as unpaid history accumulates. Candidates whose last successful send is already today (WIB) are skipped, so an invoice Phase 1 sent from an older row is not chased again in a later slot. The rest are ordered **least-recently-contacted first** (never-chased ahead of all, then oldest send, tie-broken by id), so an invoice cut off by a limit leads the next run instead of the same prefix being chased every time.
...
curl -X POST http://localhost:3000/api/cron/send-reminders \
  ...
  -d '{"slot": 10}'   # Phase 1 remainder + overdue chase
                     # add "batchLimit": 3 for a controlled canary run
