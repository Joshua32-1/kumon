...
3. **`send-reminders`** (1st/11th/21st, ten half-hour slots 09:00–13:30 WIB) — each slot stops at whichever comes first: `WHATSAPP_BATCH_LIMIT` (default 200) sends or `REMINDER_RUN_BUDGET_MS` (4.5 min) of wall clock, with `WHATSAPP_SEND_DELAY_MS` (default 500 ms) between sends. The time budget is the real governor and sits under the route's 300 s `maxDuration` so a full slot returns `truncated: true` instead of being killed mid-loop with its result lost. Phase 1 (every slot) handles each invoice that has a due (`scheduled_date <= today`), not-yet-sent reminder and sends only its **latest** due reminder, cancelling earlier due rows ("Digantikan pengingat terbaru") — so a reminder stranded in the past (mid-month enrollment / cuti rebill) is sent once and in order, never duplicated or mislabeled. `PENDING` rows have no lower bound, so they self-heal on any later run; a `FAILED` row is retried for `REMINDER_FAILED_RETRY_WINDOW_DAYS` (11) measured from the **later** of `first_failed_on` (the day the send was actually attempted) and `scheduled_date` — its own remaining slots plus the next cron run, and no further. Anchoring to the attempt matters because the two diverge when the schedule and the cron cadence drift (a `reminder_days` config `vercel.json` does not follow): a schedule-anchored window could expire before the next run and strand the row. Taking the later of the two matters because a failure can also land *before* the row is due — the admin bulk push-ahead sends the earliest future reminder — and anchoring on that alone would expire the window before the row ever came due. The anchor is stamped once and never refreshed, so repeat failures cannot slide it forward. The window is what makes a mid-batch failure durable (scoped to the failure's own day, a row that failed during an outage matched nothing once the day rolled over, and the parent silently never got that reminder) while still bounding how long a permanently-failing number is re-attempted. It costs no extra messages: an invoice is sent at most once per run and only its latest due reminder, so a rediscovered `FAILED` row that a newer due reminder has overtaken is cancelled rather than re-sent, and one already covered by Phase 2 is claimed by Phase 1 first instead of twice — Phase 2 skips any invoice whose last successful send is already today (WIB), which is what holds "one message per invoice per day" across the ten slots, since `processedInvoiceIds` only spans a single invocation and the catch-up writer recognises a prior send only from a row dated today. Nor does it cost capacity: discovery inner-joins `invoices` and keeps only `PENDING`/`OVERDUE` ones — exactly the statuses a send would act on — so reminders left behind on a settled invoice never take a batch slot. Slots 3–10 additionally chase overdue/prior-month invoices (Phase 2); Phase 1 always drains first within an invocation, so the chase only spends leftover budget. Phase 2 reaches back at most `REMINDER_CHASE_MAX_PRIOR_MONTHS` (3) periods and orders candidates **least-recently-contacted first**, so a partial run rotates rather than re-chasing the same prefix and starving the tail. The slot is inferred from WIB clock time or passed explicitly. Already-`SENT` rows are skipped, so slots dedupe naturally. Constants in [lib/constants.ts](lib/constants.ts).
...
4. Admin actions: **Batalkan** (cancel + best-effort expire the Snap order), **Hitung ulang tagihan** (recalculate line items, expire old session — same link opens a fresh checkout), **Tandai Lunas** (manual cash override, no auto-confirmation message), **Kirim konfirmasi pembayaran** (explicit confirmation send).
   **Kirim Link via WhatsApp** bulk-sends a period's unsent links through the same per-invoice path as the cron (`sendPaymentReminderForInvoice`), and is governed the same way: it stops at `WHATSAPP_BATCH_LIMIT` sends or `PAYMENT_LINK_RUN_BUDGET_MS` of wall clock, whichever comes first, and returns `truncated: true` with its counts. The budget matters more here than in the cron because there is a human waiting — being killed at `maxDuration` mid-loop would show the admin a failed request while messages had in fact gone out — and it is tighter than the cron's (240 s vs 270 s) because this path also pays for `revalidatePath` and the RSC re-render *after* the loop returns. `maxDuration = 300` for this path is carried by [app/(dashboard)/payments/layout.tsx](app/\(dashboard\)/payments/layout.tsx): the Pembayaran page is a client component and so cannot export route segment config itself.
